...
fn main() -> Result<()> {
    ...
    for package in selected.iter() {
        ...
        let mut modules = cargo_expand(package, &features)?;
        compile(package, &dependencies, &mut modules)?;
        cache(package, &modules)?;
...
struct ModuleData {
    ...
    kernel_descs: HashMap<String, KernelDesc>,
}
...
fn cache(package: &Package, modules: &HashMap<String, ModuleData>) -> Result<()> {
    use std::fmt::Write;
    let mut cache = "macro_rules! __krnl_module {\n".to_string();
    for module in modules.values() {
        writeln!(
            &mut cache,
            "({name}) => {{ macro_rules! __krnl_kernel {{",
            name = module.name_with_hash
        )
        .unwrap();
        for (kernel, kernel_desc) in module.kernel_descs.iter() {
            let bytes = bincode::serialize(kernel_desc)?;
            writeln!(
                &mut cache,
                r#"        ({kernel}) => {{
                &{bytes:?}
...
fn compile(
    package: &Package,
    dependencies: &str,
    modules: &mut HashMap<String, ModuleData>,
) -> Result<()> {
    use std::fmt::Write;
...
"#
    ...
    }
    {
        ...
        for (entry_point, spirv_path) in spirv_modules.iter() {
            ...
            let module = modules.get_mut(module_name_with_hash).unwrap();
            ...
            module
                .kernel_descs
                .insert(kernel_desc.name.clone(), kernel_desc);
        }
    }
    Ok(())
}
