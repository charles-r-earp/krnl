...
fn clean(workspace: Workspace, manifest: Manifest) -> Result<()> {
    ...
    if krnl_dir.exists() {
        ...
        if packages_dir.exists() {
            ...
            if fs::read_dir(&packages_dir)?.next().is_none() {
                fs::remove_dir(&packages_dir)?;
            }
        }
        if !packages_dir.exists() {
            ...
            if cachedir_tag_path.exists() {
                fs::remove_file(&cachedir_tag_path)?;
            }
            let gitignore_path = krnl_dir.join(".gitignore");
            if gitignore_path.exists() {
                fs::remove_file(&gitignore_path)?;
            }
            fs::remove_dir(&krnl_dir)?;
        }
    }
    Ok(())
}
