...
fn cache(package: &Package, kernels: Vec<KernelDesc>, check: bool) -> Result<()> {
    ...
    let header = format!("/* generated by krnlc {version} */\n");
    let prefix = "__krnl_cache!(r#\"\n";
    let suffix = "\"#);";
    let mut chunks = bytes.chunks_exact(800);
    let encoded_len = chunks.len() + (bytes.len() / 4 + chunks.remainder().is_empty() as usize) * 5;
    let mut cache = String::with_capacity(header.len() + prefix.len() + encoded_len + suffix.len());
    cache.push_str(&header);
